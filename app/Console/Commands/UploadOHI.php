...
class UploadOHI extends Command
{
    private $OHIAPI;
     ...
     * @var string
     */
    protected $signature = 'upload:OHI';
     ...
     * @var string
     */
    protected $description = 'Send data to OHI';
    ...
    public function handle()
    {
        ...
        $this->removeProcessedLinks();

        $this->updateOhiStatus();
    }
     ...
     * @return Array
     */
    private function removeProcessedLinks () {

        CampaignLink::whereNotNull('added_to_ohi_at')->delete();
        Campaign::where('ohi_batch_state', $this->completedOHIState)->delete();
     ...
     * @return Array
     */
    private function updateOhiStatus () {

        $ohi = OhiStatus::where('completed_at',  $this->today)->first();

        if($ohi == null) {
            $ohi = new OhiStatus();
            $ohi->completed_at = $this->today;            
        }

        $ohi->for_process = CampaignLink::count();
        $ohi->processed = $this->totalAddedToday;
        $ohi->save();
